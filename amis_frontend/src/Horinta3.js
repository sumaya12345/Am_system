...
function Horinta3Dashboard({ user, onLogout }) {
  ...
  return (
    <div className="dashboard-container" style={{ display: 'flex' }}>
      ...
      <aside className="no-print" style={dynamicSidebarStyle}>
        {/* BUTTON-KA OO SIDEBAR-KA GUDIHIISA AH */}
        <div 
          onClick={() => setIsSidebarOpen(!isSidebarOpen)}            style={{ 
            padding: '18px 20px', 
            cursor: 'pointer', 
            fontSize: '20px', 
        ...
        <div style={{ ...logoArea, padding: isSidebarOpen ? '20px 15px' : '10px' }}>
          {isSidebarOpen ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div style={{ textAlign: 'left' }}>
                <h3 style={{ color: '#ffffff', margin: 0, fontSize: '14px', fontWeight: '700' }}>
                  {activeUser.username || "H3 Officer"}
                </h3>
                <p style={{ color: '#93c5fd', fontSize: '11px', margin: '2px 0 0 0', fontWeight: '600' }}>Role: {activeUser.role || 'H3'}</p>
              </div>
            </div>
          ) : (
            <h2 style={{ color: '#93c5fd', fontSize: '14px', fontWeight: '700' }}>H3</h2>
          )}
        </div>

        <nav style={{ marginTop: '20px' }}>
          <div onClick={() => setActiveTab('dashboard')} style={activeTab === 'dashboard' ? navActive : navItem}>
            <span>📊</span> {isSidebarOpen && <span style={{ marginLeft: '10px' }}>Dashboard</span>}
          </div>
          <div onClick={() => setActiveTab('personnel')} style={activeTab === 'personnel' || activeTab === 'history-view' ? navActive : navItem}>
            <span>👥</span> {isSidebarOpen && <span style={{ marginLeft: '10px' }}>Xogta Guud</span>}
          </div>
          <div onClick={() => setActiveTab('reports')} style={activeTab === 'reports' ? navActive : navItem}><span>📄</span>{isSidebarOpen && <span style={{ marginLeft: '10px' }}>Reports</span>}</div>
          <div onClick={() => setActiveTab('analytics')} style={activeTab === 'analytics' ? navActive : navItem}><span>📊</span>{isSidebarOpen && <span style={{ marginLeft: '10px' }}>Analytics</span>}</div>
          <div onClick={() => setShowMsgModal(true)} style={showMsgModal ? navActive : navItem}>
            <span>💬</span> {isSidebarOpen && <span style={{ marginLeft: '10px' }}>Fariimaha</span>}
          </div>
        </nav>
      ...
      <main style={dynamicMainContentStyle} className="main-content">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }} className="no-print">
            <h1 style={{ margin: 0, color: '#1e293b', fontSize: '22px', fontWeight: '700' }}>H3 Dashboard</h1>
            <button onClick={fetchData} style={refreshBtn}>🔄 Refresh H3 Data</button>
        </div>
       ...
       {activeTab === 'dashboard' && (
        ...
        {activeTab === 'personnel' && (
          <div style={cardStyle}>
            <h2 style={{ color: '#1a2a6c', marginBottom: '20px' }}>Xogta Guud ee Askarta Horinta 3aad</h2>
                ...
                <tbody>
                  {personnel.map(p => (
                    <tr key={p.id} style={trStyle}>
                      ...
                      <td>{new Date(p.tariikhda_dhalashada).toLocaleDateString()}</td> 
          ...
          <div>
            ...
            <div style={profileHeaderCard}>
              ...
              <div style={{ marginLeft: '25px' }}>
                ...
                <p style={{ margin: '5px 0', color: '#666' }}>Sarkaal ID: <strong>{selectedStaff.sarkaal_id}</strong></p>
                <span style={visitBadge}>Visits Count: {medicalHistory.length}</span>
              </div>
            </div>

            <div style={{ ...cardStyle, marginTop: '20px' }}>
              <h3 style={{ borderBottom: '2px solid #f1c40f', display: 'inline-block', paddingBottom: '5px', marginBottom: '20px' }}>Taariikhda Baaritaanada</h3>
              <table style={tableStyle}>
                <thead>
                  <tr style={{ textAlign: 'left', borderBottom: '2px solid #eee' }}>
                   <th style={pad15}>Date</th> <th style={pad15}>Diagnosis</th><th style={pad15}>Limitation</th><th style={pad15}>Days</th> <th style={pad15}>Referrals</th>
                  </tr>
                </thead>
                <tbody>
...
export default Horinta3Dashboard;