...
function Horinta2Dashboard({ user, onLogout }) {
  ...
  const fetchData = async () => {
    try {
      const [qRes, rRes, pRes] = await Promise.all([
        axios.get('http://localhost:5000/api/ballan/queue?user_id=2'),
        axios.get('http://localhost:5000/api/medical-records?user_id=2'),
        axios.get('http://localhost:5000/api/s2-data')
      ]);
      ...
      console.error("Xogta Horinta 2aad lama soo xiriirin karno:", err);
    }
  };
    ...
    <div className="dashboard-container" style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f4f7f9' }}>
      ...
      <aside className="no-print" style={dynamicSidebarStyle}>
        ...
        <div 
          ...
          style={{ padding: '20px', cursor: 'pointer', fontSize: '20px', textAlign: isExpanded ? 'right' : 'center', color: '#93c5fd' }}
        >
          {isExpanded ? '☰' : '☰'}
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div style={{ textAlign: 'left' }}>
                <h3 style={{ color: '#ffffff', margin: 0, fontSize: '14px', fontWeight: '700' }}>
                    {activeUser.username || "H2 Officer"}
                </h3>
                  <p style={{ color: '#93c5fd', fontSize: '11px', margin: '2px 0 0 0', fontWeight: '600' }}>Role: {activeUser.role || 'H2'}</p>
              </div>
            </div>
          ) : (
            <h2 style={{ color: '#93c5fd', fontSize: '14px', fontWeight: '700' }}>H2</h2>
          )}
        </div>
      ...
      <main style={dynamicMainContentStyle} className="main-content">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }} className="no-print">
            <h1 style={{ margin: 0, color: '#1e293b', fontSize: '22px', fontWeight: '700' }}>H2 Dashboard</h1>
               <button onClick={fetchData} style={{ ...refreshBtn, backgroundColor: '#1d4ed8' }}>🔄 Refresh H2 Data</button>
        </div>
        ...
        {activeTab === 'personnel' && (
          <div style={cardStyle}>
            <h2 style={{ color: '#1e293b', marginBottom: '20px', fontWeight: '700' }}>Xogta Guud ee Askarta Horinta 2aad</h2>
            <div style={{ overflowX: 'auto' }}>
              <table style={tableStyle}>
                <thead style={darkThead}>
                  <tr>
                    <th style={pad15}>Pic</th><th style={pad15}>ID</th><th style={pad15}>Magaca</th><th style={pad15}>Culays</th><th style={pad15}>Dhiiga</th><th style={pad15}>Dhirirka</th><th style={pad15}>Goobta Dhalashada</th><th style={pad15}>Taariikhda Dhalashada</th><th style={pad15}>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {personnel.map(p => (
                    <tr key={p.id} style={trStyle}>
                      ...
                      <td style={pad15}><button onClick={() => handleViewDetails(p)} style={viewBtn}>View History</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'history-view' && selectedStaff && (
          <div>
            <button onClick={() => setActiveTab('personnel')} style={{ marginBottom: '15px', border: 'none', background: 'none', cursor: 'pointer', color: '#1a2a6c', fontWeight: 'bold', fontSize: '16px' }}>⬅ Ka Noqo</button>
            <div style={profileHeaderCard}>
              <img src={`http://localhost:5000/${selectedStaff.profile_pic}`} style={profileImageLg} alt=""/>
              <div style={{ marginLeft: '25px' }}>
                <h1 style={{ margin: 0, color: '#1e293b' }}>{selectedStaff.name}</h1>
                <p style={{ margin: '5px 0' }}>Sarkaal ID: <strong>{selectedStaff.sarkaal_id}</strong></p>
                <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                    <span style={visitBadge}>Dhiigga: {selectedStaff.dhiiga}</span>
                    <span style={visitBadge}>Records: {medicalHistory.length}</span>
                </div>
              </div>
            </div>
              ...
              <table style={tableStyle}>
                <thead>
                  <tr style={{ textAlign: 'left', borderBottom: '2px solid #eee' }}>
                    <th style={pad15}>Date</th> <th style={pad15}>Diagnosis (Xanuunka)</th><th style={pad15}>Limitation</th><th style={pad15}>Days</th> <th style={pad15}>Referrals</th>
                  </tr>
                </thead>
                <tbody>
                  {medicalHistory.map((h, i) => (
                    <tr key={i} style={trStyle}>
                      ...
                      <td style={pad15}>{h.limitations || 'None'}</td>
                      <td style={pad15}><span style={daysLabel}>{h.days} Maalmood</span></td>
                      <td style={pad15}>
  {h.referrals === 'Yes' ? (
    <span style={{ 
      color: '#5bc0de', 
...
const visitBadge = { backgroundColor: '#dbeafe', padding: '5px 14px', borderRadius: '20px', fontSize: '13px', color: '#1d4ed8', fontWeight: '600' };
...
export default Horinta2Dashboard;